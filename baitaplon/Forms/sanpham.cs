...
namespace baitaplon.Forms
{
    public partial class sanpham : Form
    {
        ...
        {   string sqlTimKiem = "";
            ...
            if (danhmucSPTimKiem.Text == "nuocngot" && txtTimKiem.Text != "")
            {
                sqlTimKiem = "SELECT * FROM SANPHAM WHERE TenSanPham like '%" + txtTimKiem.Text + "%' AND DanhMuc = '"+danhmucSPTimKiem.Text+"'";
                bunifuCustomDataGrid1.DataSource = cnn.XemDL(sqlTimKiem);
            }
            if (danhmucSPTimKiem.Text == "banhmi" && txtTimKiem.Text != "")
            {
                sqlTimKiem = "SELECT * FROM SANPHAM WHERE TenSanPham like '%" + txtTimKiem.Text + "%' AND DanhMuc = '" + danhmucSPTimKiem.Text + "'";
                bunifuCustomDataGrid1.DataSource = cnn.XemDL(sqlTimKiem);
            }
            if (danhmucSPTimKiem.Text == "mitom" && txtTimKiem.Text != "")
            {
                sqlTimKiem = "SELECT * FROM SANPHAM WHERE TenSanPham like '%" + txtTimKiem.Text + "%' AND DanhMuc = '" + danhmucSPTimKiem.Text + "'";
                bunifuCustomDataGrid1.DataSource = cnn.XemDL(sqlTimKiem);
            }
            if (danhmucSPTimKiem.Text == "sua" && txtTimKiem.Text != "")
            {
                sqlTimKiem = "SELECT * FROM SANPHAM WHERE TenSanPham like '%" + txtTimKiem.Text + "%' AND DanhMuc = '" + danhmucSPTimKiem.Text + "'";
                bunifuCustomDataGrid1.DataSource = cnn.XemDL(sqlTimKiem);
            }
            if (danhmucSPTimKiem.Text == "thit" && txtTimKiem.Text != "")
            {
                sqlTimKiem = "SELECT * FROM SANPHAM WHERE TenSanPham like '%" + txtTimKiem.Text + "%' AND DanhMuc = '" + danhmucSPTimKiem.Text + "'";
                bunifuCustomDataGrid1.DataSource = cnn.XemDL(sqlTimKiem);
            }
            if (danhmucSPTimKiem.Text == "rau" && txtTimKiem.Text != "")
            {
                sqlTimKiem = "SELECT * FROM SANPHAM WHERE TenSanPham like '%" + txtTimKiem.Text + "%' AND DanhMuc = '" + danhmucSPTimKiem.Text + "'";
                bunifuCustomDataGrid1.DataSource = cnn.XemDL(sqlTimKiem);
            }
            if (danhmucSPTimKiem.Text == "hoaqua" && txtTimKiem.Text != "")
            {
                sqlTimKiem = "SELECT * FROM SANPHAM WHERE TenSanPham like '%" + txtTimKiem.Text + "%' AND DanhMuc = '" + danhmucSPTimKiem.Text + "'";
                bunifuCustomDataGrid1.DataSource = cnn.XemDL(sqlTimKiem);
            }
            if (danhmucSPTimKiem.Text == "man" && txtTimKiem.Text != "")
            {
                sqlTimKiem = "SELECT * FROM SANPHAM WHERE TenSanPham like '%" + txtTimKiem.Text + "%' AND DanhMuc = '" + danhmucSPTimKiem.Text + "'";
                bunifuCustomDataGrid1.DataSource = cnn.XemDL(sqlTimKiem);
            }
            if (danhmucSPTimKiem.Text == "dao" && txtTimKiem.Text != "")
            {
                sqlTimKiem = "SELECT * FROM SANPHAM WHERE TenSanPham like '%" + txtTimKiem.Text + "%' AND DanhMuc = '" + danhmucSPTimKiem.Text + "'";
                bunifuCustomDataGrid1.DataSource = cnn.XemDL(sqlTimKiem);
            }
        ...
        private void danhmucSP_SelectedIndexChanged(object sender, EventArgs e)
        {
